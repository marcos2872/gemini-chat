...
Após o *Token Exchange*, todas as chamadas usam a `endpoints.api` retornada.
...
- **Ferramentas (MCP)**:
  - O `AuthController` injeta ferramentas MCP convertidas para o formato OpenAI Function Calling.
  - Execução de ferramentas acontece no `AuthController.ts` e retorna via mensagens de role `tool`.
...
| **Controller** | `src/boot/controllers/AuthController.ts` | Ponte IPC, gerencia ciclo de vida e integração com MCP. |
