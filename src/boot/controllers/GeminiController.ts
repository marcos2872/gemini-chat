...
export class GeminiController {
  ...
  private registerRoutes() {
    this.router.registerHandler(
      IPC_CHANNELS.GEMINI.PROMPT,
      this.handlePrompt.bind(this)
    );
    this.router.registerHandler(IPC_CHANNELS.GEMINI.HISTORY, async () =>
      this.gemini.getHistory()
    );
    this.router.registerHandler(IPC_CHANNELS.GEMINI.LIST_MODELS, async () =>
      this.gemini.listModels()
    );

    this.router.registerHandler(
      IPC_CHANNELS.GEMINI.SET_MODEL,
    ...
    this.router.registerHandler(
      IPC_CHANNELS.GEMINI.SET_KEY,
      async (event, key: string) => {
        try {
          // Legacy or OAuth?
          // If key is provided and looks like a key, maybe keep usage?
          // But we want to enforce OAuth.
          // Let's assume this action now triggers OAuth login if we are using the new flow.

          await this.gemini.signIn();
    ...
    this.router.registerHandler(
      IPC_CHANNELS.GEMINI.CHECK_CONNECTION,
      async () => {
        try {
    ...
    this.router.registerHandler(IPC_CHANNELS.GEMINI.SIGN_OUT, async () => {
      try {
        await this.gemini.signOut();
        return { success: true };
      } catch (e: any) {
    ...
    try {
      ...
      if (!activeConversation.model) {
        activeConversation.model =
          this.gemini.modelName || "gemini-2.5-flash-lite";
      }

      // Connect MCP
      await this.mcpManager.connectAll();

      const response = await this.gemini.sendPrompt(
        prompt,
        this.mcpManager,
