...
export class McpService {
    ...
    async addServer(server: MCPServer) {
        ...
        if (server.type === 'sse') {
            ...
            try { new URL(server.url); } catch { throw new Error('Invalid URL format.'); }
        } else {
    ...
    async updateServer(name: string, updates: Partial<MCPServer>) {
        ...
        if (oldEnabled && newEnabled === false) {
            await this.connectionManager.disconnectServer(name);
        } else if ((!oldEnabled || !this.connectionManager.hasConnection(name)) && newEnabled === true) {
    ...
    async testConfig(config: MCPServer) {
        ...
        if (config.type === 'sse') {
            ...
            if (config.token) {
                ...
                opts.authProvider = {
                    tokens: async () => ({ access_token: config.token!, token_type: 'Bearer' }),
