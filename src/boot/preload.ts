...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    mcpGetPrompt: (serverName: string, promptName: string, args: Record<string, unknown>) =>
        ipcRenderer.invoke(IPC_CHANNELS.MCP.GET_PROMPT, serverName, promptName, args),
    ...
    mcpTestConfig: (config: McpServer) => ipcRenderer.invoke(IPC_CHANNELS.MCP.TEST_CONFIG, config),
    ...
    requestDeviceCode: () => ipcRenderer.invoke(IPC_CHANNELS.AUTH.REQUEST_DEVICE_CODE),
    pollForToken: (deviceCode: string, interval: number) =>
        ipcRenderer.invoke(IPC_CHANNELS.AUTH.POLL_TOKEN, deviceCode, interval),
    ...
    onConversationUpdate: (callback: (conversation: Conversation) => void): (() => void) => {
        const handler = (_event: IpcRendererEvent, conversation: Conversation) =>
    ...
    sendApprovalResponse: (approved: boolean) =>
        ipcRenderer.send(IPC_CHANNELS.GEMINI.APPROVAL_RESPONSE, {
            approved,
