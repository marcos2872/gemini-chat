...
export const GitHubAuthModal: React.FC<GitHubAuthModalProps> = ({ isOpen, onClose, onAuthenticated }) => {
    const { isAuthenticating, deviceCode, error, startAuth, cancelAuth, openVerificationUri } = useCopilotAuth();
            ...
            <div style={{
                ...
                {step === 'code' && deviceCode && (
                    <>
                        ...
                        <button onClick={handleCancel} style={{ marginTop: '1rem', background: 'none', border: '1px solid #555', color: '#CCC', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer' }}>Cancel</button>
                    </>
                )}
                ...
                {step === 'error' && (
                    <div style={{ color: '#F44336' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '1rem' }}>
                            <button onClick={handleCancel} style={{ padding: '0.5rem 1rem', background: 'transparent', border: '1px solid #555', color: '#CCC', cursor: 'pointer', borderRadius: '4px' }}>Close</button>
                            <button onClick={handleStartAuth} style={{ padding: '0.5rem 1rem', background: '#333', border: '1px solid #555', color: '#FFF', cursor: 'pointer', borderRadius: '4px' }}>Try Again</button>
                        </div>
                    </div>
                )}
            </div>
