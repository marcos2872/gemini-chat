...
interface UseChatReturn {
    sendMessage: (content: string, provider: ProviderType, model: string, history?: Message[]) => Promise<string>;
    ...
    const sendMessage = useCallback(async (
        ...
        try {
            if (provider === ProviderType.GEMINI) {
                ...
                const result = await window.electronAPI.copilotChatStream(messagesPayload, model);
                if (!result.success) {
